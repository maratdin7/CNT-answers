...
- **Запрос/Ответ маски адреса (17, 18)** — *чтобы узнать топологию удаленной сети. Считается опасным, не рекомендуется к использованию вне текущей сети)*
...
- Не обеспечивает квитирование (нет подтверждения)
...
- **Смещение заголовка**
  ...
  Это поле определяет размер заголовка пакета TCP в 4-байтных (4-октетных) словах. Минимальный размер составляет 5 слов, а максимальный — 15. Смещение считается от начала заголовка TCP.
...
![Установка соединения](img/2021_12_27__02_26_38.jpg)
...
Маршрутизация является ключевой функцией **IP**, т.к. необходима для поиска маршрута доставки этого пакета.

### Маршрутизация

- Функция сетевого уровня
  - Заключается в поиске оптимального маршрута для доставки пакетов
  - Сбор и хранение информации о других маршрутизаторах и хостах в сети
- Маршрутизация:
  - Индивидуальная (два узла устанавливают между собой отдельный маршрут)
...
- Другие названия:
  ...
  - Router
  - Gateway
- Устройство сетевого уровня, реализующее функции маршрутизации
...
**Статическая маршрутизация** – вид маршрутизации, при котором маршруты указываются в явном виде при конфигурации маршрутизатора администратором. Вся маршрутизация при этом происходит без участия каких-либо протоколов маршрутизации.
...
**Автономная система (AS)** — часть узлов в сети:
- управлющихся из одного центра управления
- реализующие одну политику маршрутизации
- втрутри AS обеспечиваются одинаковые протоколы маршрутизации

### Протоколы маршрутизации внутри AS 

- Внутренние протоколы маршрутизации (**IGP** – Interior Gateway Protocol) 

### Протоколы маршрутизации между AS 

- Внешние протоколы маршрутизации (**EGP** – Exterior Gateway Protocol)
...
- **Метрика протокола** — Каким образом протокол оценивает эффективность маршрута
...
- **Сходимость** — Возможность как можно скорее настроить таблицу маршрутизации при изменениях в сети 
- Избежание петель маршрутизации 
- Загрузка сети — какую дополнительную нагрузку создает на сеть
- Ресурсоемкость — какую нагрузку создает на маршрутизаторы  
...
RIP (Routing Information Protocol) — один из самых простых протоколов маршрутизации. Использует алгоритм Беллмана-Форда

Свойства
- в небольших компьютерных сетях
- позволяет маршрутизаторам динамически обновлять маршрутную информацию, получая ее от соседних маршрутизаторов
...
Граф маршрутизации для **RIP** — Граф где все ребра 1

## Алгоритм работы

- Рассылает соседям свою таблицу маршрутизации *(ТМ)*
- Когда получаем *ТМ* изменяем свою *ТМ* 

### Рассылка 
- Каждый RIP-маршрутизатор отправляет несколько раз в секунду свою таблицу маршрутизации полную *ТМ*
- Вся таблица маршрутизации помещается в пакет
- Каждая строчка в *ТМ* — это D<sub>(j),i</sub> в терминах Беллмана-Форда

## Коррекция маршрута (таблицы):

![Коррекция таблицы маршрутизации](img/2021_12_27__16_55_07.jpg)
...
## Проблема петель в протоколе RIP

![](img/2021_12_27__17_02_16.jpg)

Оба маршрутизаторы не смогут доставялть пакеты до *N*, даже если есть альтернативный маршрут

- Для адресации соседних маршрутизаторов используется широковещательная адрересация
- Пакет RIP инкапсулируется в UDP 520
- В один пакет — до 25 маршрутных записей
Довольно сильно нагружая низкоскоростные линии связи.

## Формат пакета

![](img/2021_12_27__17_36_13.jpg)

1. Команда
  1. RIP-запрос — чтобы не ждать 30 сек после включения
  1. RIP-ответ
1. Версия — 1/2
1. Идентификатор семейства адресов — тип сетевой среды (TCP/IP)
1. IP-адрес — адрес сети класса *A*, *B*, *C*, но нет маски (поэтому протокол сейчас почти не используется)
1. Метрика — для текущего маршрута  
...
Сходимость протокола **RIP** — плохая.
...
- **Split Horizon** — расщепление горизонта 
  - Обновления маршрута не посылаются на интерфейс, из которого этот маршрут получен
- **Poison Reverse** — обратный яд 
  - Обновления маршрута посылаются на интерфейс, из которого этот маршрут получен, но с метрикой 16. Во время штатного работы такие обновления — игнорируются
- **Triggered Updates** — мгновенные обновления 
  - В случае изменения метрики маршрута маршрутизатор посылает обновления немедленно, не дожидаясь 30-секундного интервала
  - Позволяет улучшить сходимость протокола (Но проблемы не решает)
  - Лавинообразно увеличивается трафик 
- **Hold Down** 
...
- Аутентификация 
- **Маска сети**
- Групповая адресация для адресации соседних маршрутизаторов
- Метки маршрута 
...
**OSPF (Open Shortest Path First)** – протокол динамической маршрутизации, основанный на технологии отслеживания состояния канала и использующий для нахождения кратчайшего пути алгоритм Дейкстры. Стандарт – RFC 1247, версии OSPF 1, 2, 3.
...
- 56 кбит/с - 1785
- Метрика канала со скоростью передачи данных. 100 мбит/с и выше - 1

Если скорость сети большая, можно задать свое значение метрики.

**Алгоритм работы:**

1. Каждый OSPF маршрутизатор периодически рассылает информацию о себе — пакеты LSA (Link State Advertisement). В них содержится информация о подключенных каналах и их состояниях (метриках). Рассылает соседям, они другим соседям и т.д. Т.о. LSA пакеты позволяют построить всю топологию сети, необходимую в алгоритме Дейкстры.
   - Рассылка осуществляется при изменении какого-либо канала
   - В результате лавинного обмена все узлы OSPF получают информацию обо всех каналах сети

1. После получения пакетов все маршрутизаторы строят LSD – Link State Database (база данных всех состояний подключенных каналов («линков»)). Все базы, построенные разными маршрутизаторами, должны быть идентичны — условие корректного функционирования протокола

1. Далее каждый маршрутизатор по LSD строит LST – Link State Tree – дерево достижимости в соответствии с алгоритмом Дейкстры. LSD одинаковые, но **LST получатся разные**, т.к каждый маршрутизатор в качестве корня будет использовать себя. LST в дальнейшем используется для маршрутизации

## Таблица маршрутизации

- IP-адрес места назначения и маска
- Тип места назначения (сеть, граничный маршрутизатор и т.д.)
- Тип сервиса (TOS) — Если мы хотим различные маршруты для разного качества обслуживания
- Домен маршрутизации — Область OSPF можно разделить на отдельные домены
- Тип пути:
  - Внутренний
  - Межобластной
  - Внешний, ведущий к AS
- Метрика
- Следующий маршрутизатор
- Объявляющий маршрутизатор (используется для
межобластных обменов и для связей автономных систем друг
с другом).
...
- Для передачи маршрутных обновлений
используются адреса:
  - Для адресации всех OSPF-маршрутизаторов: 224.0.0.5
  - Для адресации всех назначенных OSPF-маршрутизаторов: 224.0.0.6
...
В протоколе OSPF требуется много расчетов, (количество операций на узле – N^2). Если разделить область OSPF на домены, то в каждом домене будет меньше узлов, чем в общей области (например, 250 вместо 1000) => уменьшится общее количество операций на узле.

## Достоинства
  - Гибкая метрика
  - Быстрая сходимость
  - Низкая загрузка каналов служебным трафиком
  - Отсутствие петель маршрутизации
  - Поддержка доменов маршрутизации
  - Поддержка различных маршрутов для разных типов
обслуживания
## Недостатки
- Высокие требования к ресурсам:
  - Быстродействие
  - Объемы памяти для хранения LSD, LST
...
(В Distance-Vector протоколах, маршрутизатор узнает информацию о маршрутах посредством маршрутизаторов, непосредственно подключенных в один с ним сегмент сети. То есть, маршрутизатор имеет информацию о топологии только в границах его соседних маршрутизаторов и понятия не имеет)

Основные особенности:
- Механизм обнаружения соседей
- Посылка обновлений таблиц
- Вычисление вероятных заместителей — Если основной маршрутизатор не подходит
- Алгоритм активного поиска DUAL

## Метрика

Метрика определяется следующими параметрами:
- **Задержка** (Internetwork delay)
- **Ширина полосы** (bandwidth) (1200 б/с – 10Гб/с)
- Надежность (reliability)
  - Определяется вероятностью (0-1)
- Нагрузка (load)
  - Определяется долей (0-1) 

В общем случае метрика 8 байт.

**Метрика сложнее и сильнее чем в других протоколах маршрутизации.**

Вычисляется по формулам. При этом администратор сети может изменять важность каждого параметра

Изначальное значени коэффициентов
- `K1 = K3 = 1`
- `K2 = K4 = K5 = 0`

Формула выраждается в `M = B + D`

## Обнаружение соседей

Используется для сбора информации о соседях
- Посылается пакет **Hello**. 1 раз в 5|60 сек
- По получаемым сообщениями делается вывод об имеющихся соседях

## Таблица топологии

- Предназначена для выбора маршрутов на сеть назначения
- Содержит поля:
  - *Минимальная полоса пропускания в пути* — Почему не суммарная? (так как застопорятся на участе с меньшей полосой пропускания)
  - *Общая задержка пути*
  - *Надежность пути*
  - Загрузка пути
  - Минимум MTU(maximum transfer unit) на пути
  - Текущая дистанция — дистанция данного маршрута (полная метрика маршрута)
  - Отчетная дистанция — дистанция, которую прислал узел, являющийся источником маршрута (метрика маршрута без последнего шага)
  - Источник маршрута
...
Задачи системы именования ресурсов
- **Преобразование символических имен в адреса**
- **Преобразование адресов в символические имена**
- Маршрутизация почты
- Хранения дополнительной информации об узлах

Для того чтобы не приходилось поддерживать таблицы соответствия локально. Используются **распределенные базы данных соответствия** 

## DNS

Особенности 
- Расперделенная(Хранится на разных узлах) по *Internet* доменная база имен
  - Ответственность за разные части иерархической структуры несут разные люди или организации
- Иерархическое именование ресурсов
- *Серверы DNS* поддерживают *базу DNS*. К ним можно обращаться и получать информацию

### Иерархическая структура DNS

- Общий домен верхнего уровня `.`
  - *Полный* адрес имеет в конце точку *www.microsoft.com.*
  - Без точки *относительный* 
- Внутрь домена верхнего уровня могут вкладываться домены следующего уровня
- Количество уровней иерархии неограниченно
- Полное имя узла получается восстановлением по дереву снизу вверх цепочки имен доменов, разделенных точками
...
- Хранение информации о зоне
- Обслуживание запросов об этой зоне
- Перенаправление запросов о других зонах вышестоящим или подчиненным серверам
- Кэширование информации о других зонах
...
- DNS-сервер
- Клиентские программы — работают с сервером через Resolver
- Resolver — Все запросы требующие разрешения имени проходят через Resolver 

По способу ответа на запрос 

1. Рекурсивные серверы (С которыми работают Resolver)
   - Самостоятельно выполняют весь поиск
   - Кэшируют полученную информацию 

2. Нерекурсивные серверы (Все серверы первого уровня)
...
В случае рекурсивного запроса DNS-сервер опрашивает серверы (в порядке убывания уровня зон в имени) **Прямой поиск**, пока не найдёт ответ или не обнаружит, что домен не существует. (На практике поиск начинается с наиболее близких к искомому DNS-серверов, если информация о них есть в кеше и не устарела, сервер может не запрашивать другие DNS-серверы.).
...
- Формам в BIND: `domain IN SOA ns e-mail (Serial Refresh Retry Expire DefaultTTL)`
- Пример: `school.server.ru. IN SOA ns.school.server.ru. admin.school.server.ru (2008121801 3600 900 2592000 900)`

AXFR – копируется вся зона.

## Запись о сервере эл.почты (MX)
...
- Формат в BIND: `Mail-domain IN MX priority address`
- `office IN MX 10 mail.school.server.ru.` 
